...
func TestLuCISettingsPage(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"data-openwrt2mqtt-password-button",
		...
		"Number(value) >= 5",
		"Offline time must be an integer of at least 5 seconds.",
...
func TestLuCISimplifiedChineseTranslation(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"msgstr \"离线时间（秒）\"",
		"msgid \"Offline time must be an integer of at least 5 seconds.\"",
		"msgstr \"离线时间必须为不小于 5 秒的整数。\"",
