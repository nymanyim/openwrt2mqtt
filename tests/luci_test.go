...
func TestLuCISettingsPage(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"bindOption(s.taboption('quick', form.Value, '_password'",
		...
		"bindSecondsOption(s.taboption('events', form.Value, '_offline_timeout', _('Device offline time (seconds)')",
		"Total time from the last trusted online evidence until the device disconnection event is published.",
		...
		"Number(value) >= 8",
		"Offline time must be an integer of at least 8 seconds.",
		"Publish an event after no trusted online evidence is observed for the configured total offline time.",
...
func TestLuCISimplifiedChineseTranslation(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"msgstr \"设备接入\"",
		"msgid \"Device offline time (seconds)\"",
		"msgstr \"设备离线时间（秒）\"",
		"msgid \"Total time from the last trusted online evidence until the device disconnection event is published.\"",
		"msgstr \"从最后一次可信在线证据开始，到正式发布设备断开事件为止的总时间。\"",
		"msgid \"Publish an event after no trusted online evidence is observed for the configured total offline time.\"",
		"msgstr \"从最后一次可信在线证据开始，持续达到设定的总离线时间后发布设备断开事件。\"",
		"msgid \"Offline time must be an integer of at least 8 seconds.\"",
		"msgstr \"离线时间必须为不小于 8 秒的整数。\"",
