...
func parseNeighbor(interfaceIndex int, message syscall.NetlinkMessage) *neighborObservation {
	...
	if len(message.Data) < neighborHeaderSize || message.Data[0] != syscall.AF_INET || int(int32(binary.NativeEndian.Uint32(message.Data[4:8]))) != interfaceIndex {
		return nil
	}
	...
	for offset := neighborHeaderSize; offset+syscall.SizeofRtAttr <= len(message.Data); {
		...
		if typ == ndaDestination && len(value) == net.IPv4len {
			o.ip = append(net.IP(nil), value...)
		}
		if typ == ndaLinkAddress && len(value) == 6 {
			o.mac = append(net.HardwareAddr(nil), value...)
		}
		offset += (length + 3) &^ 3
	}
...
func neighborData(interfaceName string, ip net.IP, mac net.HardwareAddr) map[string]any {
	...
	for k, v := range resolveLease(mac.String()) {
		data[k] = v
	}
	return data
}
func newEvent(routerID, interfaceName, eventType string, data map[string]any, now time.Time) event.Event {
	timestamp := now.UTC()
	...
	return event.Event{SchemaVersion: "1", ID: neighborEventID(routerID, eventType, mac, timestamp), RouterID: routerID, Category: "network", Type: eventType, Source: "neighbor/" + interfaceName, Timestamp: timestamp, Data: copied}
}
