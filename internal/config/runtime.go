...
func FromEnvironment() (Runtime, error) {
	...
	if err := parseBoolEnvironment("OPENWRT2MQTT_EVENT_DEVICE_CONNECTED_ENABLED", &config.DeviceConnectedEnabled); err != nil {
		return Runtime{}, err
	}
	if err := parseBoolEnvironment("OPENWRT2MQTT_EVENT_DEVICE_DISCONNECTED_ENABLED", &config.DeviceDisconnectedEnabled); err != nil {
		return Runtime{}, err
	}
	...
	if (config.DeviceConnectedEnabled || config.DeviceDisconnectedEnabled) && config.MQTTBroker == "" {
		return Runtime{}, errors.New("OPENWRT2MQTT_MQTT_BROKER must not be empty when device events are enabled")
	}
	return config, nil
}

func parseBoolEnvironment(key string, target *bool) error {
	value := os.Getenv(key)
	if value == "" {
		return nil
	}
